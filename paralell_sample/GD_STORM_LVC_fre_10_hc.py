...
logdir = "./GD_STORM_LVC_ps/%s/batchsize%d_innersize%d_seed%d_lrcritic%f_lractorinit%f_freq_%d" % (
    str(ENV_NAME), outer_batch, inner_batch, SEED, LR_CRITIC, LR_ACTOR_INITIAL, num_inner)
...
for j in count():
    ...
    for inner_update in range(num_inner):
        ...
        for step in range(inner_batch//num_process):
            # Sample actions
            with torch.no_grad():
                value, action, action_log_prob, recurrent_hidden_states = actor_critic.act(
