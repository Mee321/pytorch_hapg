...
class FWStormLVC:
    ...
    def update(self, rollouts):
        ...
        #######################################################
        ndarrays = get_ndarrays_from_flat(-grad)
        u_vecs = []
        for ndarray in ndarrays:
            u_vec_layer = self.LO(ndarray)
            u_vec_layer = torch.from_numpy(u_vec_layer).view(-1)
            u_vecs.append(u_vec_layer)
        u_vec = torch.cat(u_vecs)
        #######################################################

        prev_params = get_flat_params_from(self.actor_critic)
        direction = u_vec - prev_params
